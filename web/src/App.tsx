  ...
  return (
    <>
      ...
      <section className="px-5 py-4">
        <div className="mx-auto grid max-w-5xl gap-3 rounded-[1.5rem] bg-psf-surface p-4 shadow-card sm:p-5 md:grid-cols-[1fr_0.8fr_auto]">
          ...
          {props.lookupMessage && <p className="break-words text-sm font-semibold text-psf-secondary md:col-span-3">{props.lookupMessage}</p>}
        </div>
      </section>
...
function MatchCard({ match, draft, now, publicPredictions, onChange, onReveal }: { match: MatchSnapshot; draft?: ScoreDraft; now: number; publicPredictions?: { loading?: boolean; predictions?: PublicPredictionSnapshot[]; error?: string }; onChange: (matchExternalId: string, side: 'homeScore' | 'awayScore', value: string) => void; onReveal: (matchExternalId: string) => void }) {
  ...
  return (
    <article className={`rounded-[2rem] bg-psf-surface p-5 shadow-card ${draft?.saved ? 'ring-2 ring-psf-success' : ''} ${disabled ? 'opacity-90' : ''}`}>
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_auto_1fr] sm:items-center"><TeamBlock name={match.homeTeam.name} logoUrl={match.homeTeam.logoUrl} align="right" /><div className="grid grid-cols-[auto_auto_auto] items-center justify-center gap-2 sm:grid-cols-[4rem_auto_4rem]"><ScoreInput value={locked ? match.homeScore : draft?.homeScore} disabled={disabled} onChange={(value) => onChange(match.externalId, 'homeScore', value)} /><span className="text-xl font-black text-psf-muted">×</span><ScoreInput value={locked ? match.awayScore : draft?.awayScore} disabled={disabled} onChange={(value) => onChange(match.externalId, 'awayScore', value)} /></div><TeamBlock name={match.awayTeam.name} logoUrl={match.awayTeam.logoUrl} align="left" /></div>
...
function TeamBlock({ name, logoUrl, align }: { name: string; logoUrl: string | null; align: 'left' | 'right' }) {
  return <div className={`flex min-w-0 items-center justify-center gap-3 ${align === 'right' ? 'sm:justify-end sm:text-right' : 'sm:justify-start'}`}>{align === 'right' && <strong className="min-w-0 truncate text-base font-black sm:text-lg">{name}</strong>}<div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full bg-psf-background text-sm font-black">{logoUrl ? <img alt="" className="h-full w-full object-cover" src={logoUrl} /> : name.slice(0, 2).toUpperCase()}</div>{align === 'left' && <strong className="min-w-0 truncate text-base font-black sm:text-lg">{name}</strong>}</div>;
}

function ScoreInput({ value, disabled, onChange }: { value?: string | number | null; disabled: boolean; onChange: (value: string) => void }) {
  return <input className="h-14 w-14 rounded-2xl bg-psf-background text-center text-2xl font-black outline-none focus:ring-2 focus:ring-psf-blue disabled:text-psf-secondary sm:w-16" disabled={disabled} inputMode="numeric" max={20} min={0} value={value ?? ''} onChange={(event) => onChange(event.target.value)} placeholder="-" />;
}
